...
class ShowInKitsu(LauncherAction):
    ...
    def process(self, session, **kwargs):
        ...
        if folder_path:
            folder_data = ayon_api.get_folder_by_path(project_name = project_name, folder_path = folder_path)
    ...
    def get_url(
        ...
        folder_type_url = "sequences" if folder_type == "Sequence" or folder_name == "Sequences" else "shots" if folder_type == "Shot" else "assets"

        if task_id:
            # Go to task page
            # /productions/{project-id}/{asset_type}/tasks/{task_id}
            sub_url += f"/{folder_type_url}/tasks/{task_id}"

        elif folder_id and folder_type != "Folder" and folder_type_url != "sequences":
            # Go to asset or shot page
            # /productions/{project-id}/assets/{entity_id}
            # /productions/{project-id}/shots/{entity_id}
            sub_url += f"/{folder_type_url}/{folder_id}"
            ...
            sub_url += f"/{folder_type_url}"
            sub_url += f"?search={folder_name}" if folder_name not in [None, "Assets", "Sequences"] else "?search="
